...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, health, animation_list, x, y, speed):
        ...
        self.body_disspearing_time = pygame.time.get_ticks()
    ...
    def update_animation(self):
        ...
        self.image = self.animation_list[self.action][self.frame_index]
        #check if enough time has passed since the last update`xxx`xx`
        if pygame.time.get_ticks() - self.update_time > ANIMATION_COOLDOWN:
            ...
            if self.frame_index >= len(self.animation_list[self.action]):
                if self.action == 2:
                    if pygame.time.get_ticks() - self.body_disspearing_time < 1000:
